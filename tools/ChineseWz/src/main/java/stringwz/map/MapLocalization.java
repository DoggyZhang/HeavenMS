...
public class MapLocalization implements ChineseLocalization {

    private static final String TARGET_XML = "E:\\MapleStory\\HeavenMS\\tools\\ChineseWz\\wz\\client\\083\\string.wz\\Map.img.xml";
    private static final String CHINESE_XML = "E:\\MapleStory\\HeavenMS\\tools\\ChineseWz\\wz\\server\\079\\string.wz\\Map.img.xml";

    private static final String OUTPUT_FOLDER = "E:\\MapleStory\\HeavenMS\\tools\\ChineseWz\\wz\\wz_chinese";
    private static final String OUTPUT_FILE_NAME = "Map.img.xml";
    ...
    public void localize() {
        MapImgXml replaceByMapImgXml = MapParser.parse(CHINESE_XML);
        // 拿到所有的汉化信息
        HashMap<String, MapImgXml.Map> replaceByCashItemMap = replaceByMapImgXml.getMapItemMap();

        MapImgXml targetEqpImgXml = MapParser.parse(TARGET_XML);
        HashMap<String, MapImgXml.Map> targetCashItemMap = targetEqpImgXml.getMapItemMap();

        List<String> notMatchedList = new ArrayList<>();
        for (String id : targetCashItemMap.keySet()) {
            MapImgXml.Map replaceItem = replaceByCashItemMap.get(id);
            if (replaceItem == null) {
                notMatchedList.add(id);
                continue;
            }

            MapImgXml.Map targetItem = targetCashItemMap.get(id);
