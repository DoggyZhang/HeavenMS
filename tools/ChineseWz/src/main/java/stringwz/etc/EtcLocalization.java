...
public class EtcLocalization implements ChineseLocalization {

    private static final String TARGET_FILE_NAME = "Etc.img.xml";
    ...
    public void localize() {
        EtcImgXml replaceByEqpImgXml = EtcParser.parse(REPLACE_BY);
        // 拿到所有的汉化信息
        Map<String, EtcImgXml.EtcItem> replaceByEqpItemMap = replaceByEqpImgXml.getEtcItemMap();

        EtcImgXml targetEqpImgXml = EtcParser.parse(REPLACE_TARGET);
        Map<String, EtcImgXml.EtcItem> targetEqpItemMap = targetEqpImgXml.getEtcItemMap();

        List<String> notMatchedList = new ArrayList<>();
        for (String id : targetEqpItemMap.keySet()) {
            EtcImgXml.EtcItem replaceItem = replaceByEqpItemMap.get(id);
            if (replaceItem == null) {
                notMatchedList.add(id);
                continue;
            }

            EtcImgXml.EtcItem targetItem = targetEqpItemMap.get(id);
