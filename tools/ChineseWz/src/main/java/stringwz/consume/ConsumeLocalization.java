...
public class ConsumeLocalization implements ChineseLocalization {

    private static final String TARGET_FILE_NAME = "Consume.img.xml";
    ...
    public void localize() {
        ConsumeImgXml replaceByCashImgXml = ConsumeParser.parse(REPLACE_BY);
        // 拿到所有的汉化信息
        Map<String, ConsumeImgXml.Consume> replaceByCashItemMap = replaceByCashImgXml.getConsumeItemMap();

        ConsumeImgXml targetEqpImgXml = ConsumeParser.parse(REPLACE_TARGET);
        Map<String, ConsumeImgXml.Consume> targetCashItemMap = targetEqpImgXml.getConsumeItemMap();

        List<String> notMatchedList = new ArrayList<>();
        for (String id : targetCashItemMap.keySet()) {
            ConsumeImgXml.Consume replaceItem = replaceByCashItemMap.get(id);
            if (replaceItem == null) {
                notMatchedList.add(id);
                continue;
            }

            ConsumeImgXml.Consume targetItem = targetCashItemMap.get(id);
